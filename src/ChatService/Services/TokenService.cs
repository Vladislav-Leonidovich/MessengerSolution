...
namespace MessageService.Services
{
    public class TokenService : ITokenService
    {
        ...
        public Task<string> GetTokenAsync()
        {
            ...
            throw new UnauthorizedAccessException("Токен авторизації відсутній у заголовках запиту.");
        }
        ...
        {
            // Спочатку пробуємо отримати токен користувача
            var userToken = await GetTokenAsync();
            if (!string.IsNullOrEmpty(userToken))
            {
                return userToken;
            ...
            // Якщо токен користувача недоступний, отримуємо міжсервісний токен
            return await GetServiceToServiceTokenAsync();
        }
